...
class FilmController extends Controller
{
    ...
    public function edit($id, Request $request)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->merge($film);      // use merge instead of persist to avoid overwriting data
            $em->flush();
            return $this->redirectToRoute("index");
        }
     ...
     * @return \Symfony\Component\HttpFoundation\Response
     */
    public function delete($id, Request $request)   // same as edit except (remove vs. merge) & different view
    {
        $film = $this->getDoctrine()->getRepository(Film::class)->find($id);
